...
type ScheduleQuery struct {
	...
	// eager-loading edges.
	withTimeSchedule                *ScheduleTimeQuery
	withWhoIsTheOwnerOfThisSchedule *UserQuery
	withFKs                         bool
...
// QueryTimeSchedule chains the current query on the "time_schedule" edge.
func (sq *ScheduleQuery) QueryTimeSchedule() *ScheduleTimeQuery {
	query := &ScheduleTimeQuery{config: sq.config}
...
// QueryWhoIsTheOwnerOfThisSchedule chains the current query on the "who_is_the_owner_of_this_schedule" edge.
func (sq *ScheduleQuery) QueryWhoIsTheOwnerOfThisSchedule() *UserQuery {
...
func (sq *ScheduleQuery) Clone() *ScheduleQuery {
	...
	return &ScheduleQuery{
		...
		withTimeSchedule:                sq.withTimeSchedule.Clone(),
		withWhoIsTheOwnerOfThisSchedule: sq.withWhoIsTheOwnerOfThisSchedule.Clone(),
...
// WithTimeSchedule tells the query-builder to eager-load the nodes that are connected to
// the "time_schedule" edge. The optional arguments are used to configure the query builder of the edge.
func (sq *ScheduleQuery) WithTimeSchedule(opts ...func(*ScheduleTimeQuery)) *ScheduleQuery {
	query := &ScheduleTimeQuery{config: sq.config}
	for _, opt := range opts {
		opt(query)
	}
	sq.withTimeSchedule = query
	return sq
}

// WithWhoIsTheOwnerOfThisSchedule tells the query-builder to eager-load the nodes that are connected to
// the "who_is_the_owner_of_this_schedule" edge. The optional arguments are used to configure the query builder of the edge.
func (sq *ScheduleQuery) WithWhoIsTheOwnerOfThisSchedule(opts ...func(*UserQuery)) *ScheduleQuery {
	query := &UserQuery{config: sq.config}
	for _, opt := range opts {
		opt(query)
	}
	sq.withWhoIsTheOwnerOfThisSchedule = query
	return sq
}
...
func (sq *ScheduleQuery) sqlAll(ctx context.Context) ([]*Schedule, error) {
	var (
		...
		_spec       = sq.querySpec()
		loadedTypes = [2]bool{
			sq.withTimeSchedule != nil,
			sq.withWhoIsTheOwnerOfThisSchedule != nil,
		}
	)
	if sq.withWhoIsTheOwnerOfThisSchedule != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := sq.withTimeSchedule; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Schedule)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.TimeSchedule = []*ScheduleTime{}
		}
		query.withFKs = true
		query.Where(predicate.ScheduleTime(func(s *sql.Selector) {
			s.Where(sql.InValues(schedule.TimeScheduleColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.schedule_time_schedule
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "schedule_time_schedule" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "schedule_time_schedule" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.TimeSchedule = append(node.Edges.TimeSchedule, n)
		}
	}

	if query := sq.withWhoIsTheOwnerOfThisSchedule; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Schedule)
		for i := range nodes {
			if nodes[i].user_doctor_has_schedule == nil {
				continue
			}
			fk := *nodes[i].user_doctor_has_schedule
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_doctor_has_schedule" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.WhoIsTheOwnerOfThisSchedule = n
			}
		}
	}
