...
type HospitalQuery struct {
	...
	// eager-loading edges.
	withHospitalHasDoctor *UserQuery
...
// QueryHospitalHasDoctor chains the current query on the "hospital_has_doctor" edge.
func (hq *HospitalQuery) QueryHospitalHasDoctor() *UserQuery {
...
func (hq *HospitalQuery) Clone() *HospitalQuery {
	...
	return &HospitalQuery{
		...
		withHospitalHasDoctor: hq.withHospitalHasDoctor.Clone(),
...
// WithHospitalHasDoctor tells the query-builder to eager-load the nodes that are connected to
// the "hospital_has_doctor" edge. The optional arguments are used to configure the query builder of the edge.
func (hq *HospitalQuery) WithHospitalHasDoctor(opts ...func(*UserQuery)) *HospitalQuery {
	query := &UserQuery{config: hq.config}
	for _, opt := range opts {
		opt(query)
	}
	hq.withHospitalHasDoctor = query
	return hq
}
...
func (hq *HospitalQuery) sqlAll(ctx context.Context) ([]*Hospital, error) {
	...
	if query := hq.withHospitalHasDoctor; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Hospital)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.HospitalHasDoctor = []*User{}
		}
		query.withFKs = true
		query.Where(predicate.User(func(s *sql.Selector) {
			s.Where(sql.InValues(hospital.HospitalHasDoctorColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.hospital_hospital_has_doctor
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "hospital_hospital_has_doctor" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "hospital_hospital_has_doctor" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.HospitalHasDoctor = append(node.Edges.HospitalHasDoctor, n)
		}
	}
