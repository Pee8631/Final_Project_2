...
type DepartmentQuery struct {
	...
	// eager-loading edges.
	withDepartmentHasDoctor *UserQuery
...
// QueryDepartmentHasDoctor chains the current query on the "department_has_doctor" edge.
func (dq *DepartmentQuery) QueryDepartmentHasDoctor() *UserQuery {
...
func (dq *DepartmentQuery) Clone() *DepartmentQuery {
	...
	return &DepartmentQuery{
		...
		withDepartmentHasDoctor: dq.withDepartmentHasDoctor.Clone(),
...
// WithDepartmentHasDoctor tells the query-builder to eager-load the nodes that are connected to
// the "department_has_doctor" edge. The optional arguments are used to configure the query builder of the edge.
func (dq *DepartmentQuery) WithDepartmentHasDoctor(opts ...func(*UserQuery)) *DepartmentQuery {
	query := &UserQuery{config: dq.config}
	for _, opt := range opts {
		opt(query)
	}
	dq.withDepartmentHasDoctor = query
	return dq
}
...
func (dq *DepartmentQuery) sqlAll(ctx context.Context) ([]*Department, error) {
	var (
		...
		_spec       = dq.querySpec()
		loadedTypes = [1]bool{
			dq.withDepartmentHasDoctor != nil,
		}
	)
	...
	if query := dq.withDepartmentHasDoctor; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Department)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.DepartmentHasDoctor = []*User{}
		}
		query.withFKs = true
		query.Where(predicate.User(func(s *sql.Selector) {
			s.Where(sql.InValues(department.DepartmentHasDoctorColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.department_department_has_doctor
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "department_department_has_doctor" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "department_department_has_doctor" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.DepartmentHasDoctor = append(node.Edges.DepartmentHasDoctor, n)
		}
	}
