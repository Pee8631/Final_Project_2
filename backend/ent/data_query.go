...
type DataQuery struct {
	...
	// eager-loading edges.
	withWhoIsTheOwnerOfThisData *UserQuery
	withFKs                     bool
...
// QueryWhoIsTheOwnerOfThisData chains the current query on the "who_is_the_owner_of_this_data" edge.
func (dq *DataQuery) QueryWhoIsTheOwnerOfThisData() *UserQuery {
...
// Returns a *NotSingularError when exactly one Data entity is not found.
...
// Returns a *NotSingularError when exactly one Data ID is not found.
...
func (dq *DataQuery) Clone() *DataQuery {
	...
	return &DataQuery{
		...
		withWhoIsTheOwnerOfThisData: dq.withWhoIsTheOwnerOfThisData.Clone(),
...
// WithWhoIsTheOwnerOfThisData tells the query-builder to eager-load the nodes that are connected to
// the "who_is_the_owner_of_this_data" edge. The optional arguments are used to configure the query builder of the edge.
func (dq *DataQuery) WithWhoIsTheOwnerOfThisData(opts ...func(*UserQuery)) *DataQuery {
	query := &UserQuery{config: dq.config}
	for _, opt := range opts {
		opt(query)
	}
	dq.withWhoIsTheOwnerOfThisData = query
	return dq
}
...
//
//	client.Data.Query().
//		GroupBy(data.FieldIdCardNumber).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
func (dq *DataQuery) sqlAll(ctx context.Context) ([]*Data, error) {
	var (
		...
		_spec       = dq.querySpec()
		loadedTypes = [1]bool{
			dq.withWhoIsTheOwnerOfThisData != nil,
		}
	)
	if dq.withWhoIsTheOwnerOfThisData != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := dq.withWhoIsTheOwnerOfThisData; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Data)
		for i := range nodes {
			if nodes[i].user_user_has_data == nil {
				continue
			}
			fk := *nodes[i].user_user_has_data
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_user_has_data" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.WhoIsTheOwnerOfThisData = n
			}
		}
	}
