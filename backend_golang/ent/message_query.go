...
type MessageQuery struct {
	...
	// eager-loading edges.
	withWhatMessagesAreInThisChat *ChatQuery
	withWhoSendMessages           *UserQuery
	withFKs                       bool
...
// QueryWhatMessagesAreInThisChat chains the current query on the "What_messages_are_in_this_chat" edge.
func (mq *MessageQuery) QueryWhatMessagesAreInThisChat() *ChatQuery {
...
// QueryWhoSendMessages chains the current query on the "Who_send_messages" edge.
func (mq *MessageQuery) QueryWhoSendMessages() *UserQuery {
...
func (mq *MessageQuery) Clone() *MessageQuery {
	...
	return &MessageQuery{
		...
		withWhatMessagesAreInThisChat: mq.withWhatMessagesAreInThisChat.Clone(),
		withWhoSendMessages:           mq.withWhoSendMessages.Clone(),
...
// WithWhatMessagesAreInThisChat tells the query-builder to eager-load the nodes that are connected to
// the "What_messages_are_in_this_chat" edge. The optional arguments are used to configure the query builder of the edge.
func (mq *MessageQuery) WithWhatMessagesAreInThisChat(opts ...func(*ChatQuery)) *MessageQuery {
	query := &ChatQuery{config: mq.config}
	for _, opt := range opts {
		opt(query)
	}
	mq.withWhatMessagesAreInThisChat = query
	return mq
}

// WithWhoSendMessages tells the query-builder to eager-load the nodes that are connected to
// the "Who_send_messages" edge. The optional arguments are used to configure the query builder of the edge.
func (mq *MessageQuery) WithWhoSendMessages(opts ...func(*UserQuery)) *MessageQuery {
	query := &UserQuery{config: mq.config}
	for _, opt := range opts {
		opt(query)
	}
	mq.withWhoSendMessages = query
	return mq
}
...
//		GroupBy(message.FieldMessageText).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
//
//	client.Message.Query().
//		Select(message.FieldMessageText).
//		Scan(ctx, &v)
//
...
func (mq *MessageQuery) sqlAll(ctx context.Context) ([]*Message, error) {
	var (
		...
		_spec       = mq.querySpec()
		loadedTypes = [2]bool{
			mq.withWhatMessagesAreInThisChat != nil,
			mq.withWhoSendMessages != nil,
		}
	)
	if mq.withWhatMessagesAreInThisChat != nil || mq.withWhoSendMessages != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := mq.withWhatMessagesAreInThisChat; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Message)
		for i := range nodes {
			if nodes[i].chat_chat_message == nil {
				continue
			}
			fk := *nodes[i].chat_chat_message
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "chat_chat_message" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.WhatMessagesAreInThisChat = n
			}
		}
	}

	if query := mq.withWhoSendMessages; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Message)
		for i := range nodes {
			if nodes[i].user_user_send_message == nil {
				continue
			}
			fk := *nodes[i].user_user_send_message
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_user_send_message" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.WhoSendMessages = n
			}
		}
	}
