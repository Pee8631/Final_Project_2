...
type ScheduleQuery struct {
	...
	// eager-loading edges.
	withScheduleAppointment *AppointmentQuery
	withScheduleDoctor      *UserQuery
	withFKs                 bool
...
// QueryScheduleAppointment chains the current query on the "schedule_appointment" edge.
func (sq *ScheduleQuery) QueryScheduleAppointment() *AppointmentQuery {
	query := &AppointmentQuery{config: sq.config}
...
// QueryScheduleDoctor chains the current query on the "schedule_doctor" edge.
func (sq *ScheduleQuery) QueryScheduleDoctor() *UserQuery {
...
func (sq *ScheduleQuery) Clone() *ScheduleQuery {
	...
	return &ScheduleQuery{
		...
		withScheduleAppointment: sq.withScheduleAppointment.Clone(),
		withScheduleDoctor:      sq.withScheduleDoctor.Clone(),
...
// WithScheduleAppointment tells the query-builder to eager-load the nodes that are connected to
// the "schedule_appointment" edge. The optional arguments are used to configure the query builder of the edge.
func (sq *ScheduleQuery) WithScheduleAppointment(opts ...func(*AppointmentQuery)) *ScheduleQuery {
	query := &AppointmentQuery{config: sq.config}
	for _, opt := range opts {
		opt(query)
	}
	sq.withScheduleAppointment = query
	return sq
}

// WithScheduleDoctor tells the query-builder to eager-load the nodes that are connected to
// the "schedule_doctor" edge. The optional arguments are used to configure the query builder of the edge.
func (sq *ScheduleQuery) WithScheduleDoctor(opts ...func(*UserQuery)) *ScheduleQuery {
	query := &UserQuery{config: sq.config}
	for _, opt := range opts {
		opt(query)
	}
	sq.withScheduleDoctor = query
	return sq
}
...
func (sq *ScheduleQuery) sqlAll(ctx context.Context) ([]*Schedule, error) {
	var (
		...
		_spec       = sq.querySpec()
		loadedTypes = [2]bool{
			sq.withScheduleAppointment != nil,
			sq.withScheduleDoctor != nil,
		}
	)
	if sq.withScheduleDoctor != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := sq.withScheduleAppointment; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Schedule)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.ScheduleAppointment = []*Appointment{}
		}
		query.withFKs = true
		query.Where(predicate.Appointment(func(s *sql.Selector) {
			s.Where(sql.InValues(schedule.ScheduleAppointmentColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.schedule_schedule_appointment
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "schedule_schedule_appointment" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "schedule_schedule_appointment" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.ScheduleAppointment = append(node.Edges.ScheduleAppointment, n)
		}
	}

	if query := sq.withScheduleDoctor; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Schedule)
		for i := range nodes {
			if nodes[i].user_doctor_schedule == nil {
				continue
			}
			fk := *nodes[i].user_doctor_schedule
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_doctor_schedule" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.ScheduleDoctor = n
			}
		}
	}
