...
// The PInfoFunc type is an adapter to allow the use of ordinary
// function as PInfo mutator.
type PInfoFunc func(context.Context, *ent.PInfoMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f PInfoFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.PInfoMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.PInfoMutation", m)
	}
	return f(ctx, mv)
}
