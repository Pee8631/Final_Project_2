...
type UserQuery struct {
	...
	withUserHasPInfo           *PInfoQuery
	withDoctorSchedule         *ScheduleQuery
	...
	withUserSendMessage        *MessageQuery
	withUserSendNotification   *NotificationQuery
...
// QueryUserHasPInfo chains the current query on the "user_has_PInfo" edge.
func (uq *UserQuery) QueryUserHasPInfo() *PInfoQuery {
	query := &PInfoQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryDoctorSchedule chains the current query on the "doctor_schedule" edge.
func (uq *UserQuery) QueryDoctorSchedule() *ScheduleQuery {
...
// QueryUserSendMessage chains the current query on the "user_send_message" edge.
func (uq *UserQuery) QueryUserSendMessage() *MessageQuery {
...
// QueryUserSendNotification chains the current query on the "user_send_notification" edge.
func (uq *UserQuery) QueryUserSendNotification() *NotificationQuery {
...
// QueryWhoIsInThisChat chains the current query on the "who_is_in_this_chat" edge.
func (uq *UserQuery) QueryWhoIsInThisChat() *ChatQuery {
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withUserHasPInfo:           uq.withUserHasPInfo.Clone(),
		withDoctorSchedule:         uq.withDoctorSchedule.Clone(),
		...
		withUserSendMessage:        uq.withUserSendMessage.Clone(),
		withUserSendNotification:   uq.withUserSendNotification.Clone(),
		...
		withWhoIsInThisChat:        uq.withWhoIsInThisChat.Clone(),
...
// WithUserHasPInfo tells the query-builder to eager-load the nodes that are connected to
// the "user_has_PInfo" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithUserHasPInfo(opts ...func(*PInfoQuery)) *UserQuery {
	query := &PInfoQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withUserHasPInfo = query
	return uq
}

// WithDoctorSchedule tells the query-builder to eager-load the nodes that are connected to
// the "doctor_schedule" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithDoctorSchedule(opts ...func(*ScheduleQuery)) *UserQuery {
	query := &ScheduleQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withDoctorSchedule = query
	return uq
}
...
// WithUserSendMessage tells the query-builder to eager-load the nodes that are connected to
// the "user_send_message" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithUserSendMessage(opts ...func(*MessageQuery)) *UserQuery {
	query := &MessageQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withUserSendMessage = query
	return uq
}

// WithUserSendNotification tells the query-builder to eager-load the nodes that are connected to
// the "user_send_notification" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithUserSendNotification(opts ...func(*NotificationQuery)) *UserQuery {
	query := &NotificationQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withUserSendNotification = query
	return uq
}
...
// WithWhoIsInThisChat tells the query-builder to eager-load the nodes that are connected to
// the "who_is_in_this_chat" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithWhoIsInThisChat(opts ...func(*ChatQuery)) *UserQuery {
	query := &ChatQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withWhoIsInThisChat = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withUserHasPInfo; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.UserHasPInfo = []*PInfo{}
		}
		query.withFKs = true
		query.Where(predicate.PInfo(func(s *sql.Selector) {
			s.Where(sql.InValues(user.UserHasPInfoColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_user_has_p_info
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_user_has_p_info" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_user_has_p_info" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.UserHasPInfo = append(node.Edges.UserHasPInfo, n)
		}
	}

	if query := uq.withDoctorSchedule; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.DoctorSchedule = []*Schedule{}
		}
		query.withFKs = true
		query.Where(predicate.Schedule(func(s *sql.Selector) {
			s.Where(sql.InValues(user.DoctorScheduleColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_doctor_schedule
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_doctor_schedule" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_doctor_schedule" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.DoctorSchedule = append(node.Edges.DoctorSchedule, n)
		}
	}
	...
	if query := uq.withUserSendMessage; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.UserSendMessage = []*Message{}
		}
		query.withFKs = true
		query.Where(predicate.Message(func(s *sql.Selector) {
			s.Where(sql.InValues(user.UserSendMessageColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_user_send_message
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_user_send_message" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_user_send_message" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.UserSendMessage = append(node.Edges.UserSendMessage, n)
		}
	}

	if query := uq.withUserSendNotification; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*User, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.UserSendNotification = []*Notification{}
		}
		var (
			edgeids []int
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(user.UserSendNotificationPrimaryKey[0], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, uq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "user_send_notification": %w`, err)
		}
		query.Where(notification.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "user_send_notification" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.UserSendNotification = append(nodes[i].Edges.UserSendNotification, n)
			}
		}
	}
	...
	if query := uq.withWhoIsInThisChat; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*User, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.WhoIsInThisChat = []*Chat{}
		}
		var (
			edgeids []int
			edges   = make(map[int][]*User)
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(user.WhoIsInThisChatPrimaryKey[1], fks...))
			},
			ScanValues: func() [2]interface{} {
				return [2]interface{}{new(sql.NullInt64), new(sql.NullInt64)}
			},
		...
		if err := sqlgraph.QueryEdges(ctx, uq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "who_is_in_this_chat": %w`, err)
		}
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "who_is_in_this_chat" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.WhoIsInThisChat = append(nodes[i].Edges.WhoIsInThisChat, n)
			}
		}
	}
