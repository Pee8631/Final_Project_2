...
type ChatQuery struct {
	...
	// eager-loading edges.
	withChatUser        *UserQuery
	withChatMessage     *MessageQuery
	withChatAppointment *AppointmentQuery
...
func (cq *ChatQuery) QueryChatUser() *UserQuery {
...
// QueryChatMessage chains the current query on the "chat_message" edge.
func (cq *ChatQuery) QueryChatMessage() *MessageQuery {
...
// QueryChatAppointment chains the current query on the "chat_appointment" edge.
func (cq *ChatQuery) QueryChatAppointment() *AppointmentQuery {
	query := &AppointmentQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *ChatQuery) Clone() *ChatQuery {
	...
	return &ChatQuery{
		...
		withChatUser:        cq.withChatUser.Clone(),
		withChatMessage:     cq.withChatMessage.Clone(),
		withChatAppointment: cq.withChatAppointment.Clone(),
...
func (cq *ChatQuery) WithChatUser(opts ...func(*UserQuery)) *ChatQuery {
	query := &UserQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withChatUser = query
	return cq
}

// WithChatMessage tells the query-builder to eager-load the nodes that are connected to
// the "chat_message" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *ChatQuery) WithChatMessage(opts ...func(*MessageQuery)) *ChatQuery {
	query := &MessageQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withChatMessage = query
	return cq
}

// WithChatAppointment tells the query-builder to eager-load the nodes that are connected to
// the "chat_appointment" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *ChatQuery) WithChatAppointment(opts ...func(*AppointmentQuery)) *ChatQuery {
	query := &AppointmentQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withChatAppointment = query
	return cq
}
...
//
//	client.Chat.Query().
//		GroupBy(chat.FieldChatRoomName).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
func (cq *ChatQuery) sqlAll(ctx context.Context) ([]*Chat, error) {
	var (
		...
		loadedTypes = [3]bool{
			cq.withChatUser != nil,
			cq.withChatMessage != nil,
			cq.withChatAppointment != nil,
		}
	)
	...
	if query := cq.withChatUser; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*Chat, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.ChatUser = []*User{}
		}
		var (
			edgeids []int
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(chat.ChatUserPrimaryKey[0], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, cq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "chat_user": %w`, err)
		}
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "chat_user" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.ChatUser = append(nodes[i].Edges.ChatUser, n)
			}
		}
	}

	if query := cq.withChatMessage; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Chat)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.ChatMessage = []*Message{}
		}
		query.withFKs = true
		query.Where(predicate.Message(func(s *sql.Selector) {
			s.Where(sql.InValues(chat.ChatMessageColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.chat_chat_message
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "chat_chat_message" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "chat_chat_message" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.ChatMessage = append(node.Edges.ChatMessage, n)
		}
	}

	if query := cq.withChatAppointment; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Chat)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.ChatAppointment = []*Appointment{}
		}
		query.withFKs = true
		query.Where(predicate.Appointment(func(s *sql.Selector) {
			s.Where(sql.InValues(chat.ChatAppointmentColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.chat_chat_appointment
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "chat_chat_appointment" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "chat_chat_appointment" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.ChatAppointment = append(node.Edges.ChatAppointment, n)
		}
	}
