...
type NotificationQuery struct {
	...
	// eager-loading edges.
	withUserNotification *UserQuery
...
// QueryUserNotification chains the current query on the "user_notification" edge.
func (nq *NotificationQuery) QueryUserNotification() *UserQuery {
...
func (nq *NotificationQuery) Clone() *NotificationQuery {
	...
	return &NotificationQuery{
		...
		withUserNotification: nq.withUserNotification.Clone(),
...
// WithUserNotification tells the query-builder to eager-load the nodes that are connected to
// the "user_notification" edge. The optional arguments are used to configure the query builder of the edge.
func (nq *NotificationQuery) WithUserNotification(opts ...func(*UserQuery)) *NotificationQuery {
	query := &UserQuery{config: nq.config}
	for _, opt := range opts {
		opt(query)
	}
	nq.withUserNotification = query
	return nq
}
...
func (nq *NotificationQuery) sqlAll(ctx context.Context) ([]*Notification, error) {
	...
	if query := nq.withUserNotification; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*Notification, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.UserNotification = []*User{}
		}
		var (
			edgeids []int
			edges   = make(map[int][]*Notification)
		)
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(notification.UserNotificationPrimaryKey[1], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, nq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "user_notification": %w`, err)
		}
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "user_notification" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.UserNotification = append(nodes[i].Edges.UserNotification, n)
			}
		}
	}
