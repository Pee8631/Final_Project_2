...
// QueryRoleUser chains the current query on the "role_user" edge.
func (rq *RoleQuery) QueryRoleUser() *UserQuery {
...
func (rq *RoleQuery) Clone() *RoleQuery {
	...
	return &RoleQuery{
		...
		withRoleUser: rq.withRoleUser.Clone(),
...
// WithRoleUser tells the query-builder to eager-load the nodes that are connected to
// the "role_user" edge. The optional arguments are used to configure the query builder of the edge.
func (rq *RoleQuery) WithRoleUser(opts ...func(*UserQuery)) *RoleQuery {
	query := &UserQuery{config: rq.config}
	for _, opt := range opts {
		opt(query)
	}
	rq.withRoleUser = query
	return rq
}
...
func (rq *RoleQuery) sqlAll(ctx context.Context) ([]*Role, error) {
	...
	if query := rq.withRoleUser; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*Role, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.RoleUser = []*User{}
		}
		var (
			edgeids []int
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(role.RoleUserPrimaryKey[0], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, rq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "role_user": %w`, err)
		}
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "role_user" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.RoleUser = append(nodes[i].Edges.RoleUser, n)
			}
		}
	}
