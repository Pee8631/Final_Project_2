...
// CheckToken handles GET requests for adding token entities
// @Summary Check token
...
// @Failure 400 {object} gin.H
// @Failure 500 {object} gin.H
// @Router /tokens/{auth_token} [POST]
func (ctl *TokenController) CheckToken(c *gin.Context) {
	obj := Token{}
	...
	VerifyToken, err := ctl.client.Token.
		Query().
		Where(token.AuthTokenEQ(obj.AuthToken)).
	...
	if err != nil {
		...
		c.JSON(400, gin.H{
			"status": false,
			"error":  "token find failed",
		})
		return
	}

	if(obj.AuthToken != VerifyToken.AuthToken){
		if err != nil {
			fmt.Println(err)
			c.JSON(400, gin.H{
				"status": false,
				"error":  "Tokens are discrepant",
			})
			return
		}
	}
...
// @Failure 500 {object} gin.H
// @Router /tokens/{auth_token} [get]
func (ctl *TokenController) GetToken(c *gin.Context) {
	Token := c.Param("auth_token")
	...
	c.JSON(200, getToken.Edges.AuthenticationToken.ID)
}
...
func (ctl *TokenController) register() {
	...
	tokens.POST(":auth_token", ctl.CheckToken)
	tokens.GET(":auth_token", ctl.GetToken)
